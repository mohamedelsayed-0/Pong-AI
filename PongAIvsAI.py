...
def init_game():
    ...
    clock_rate = 240
    turn_wait_rate = 1
    score_to_win = 1000 #change this if u just wanna test things
    ...
    paddles[0].move_getter = pong_ai.pong_ai #left is me (before switching)
    paddles[1].move_getter = chaser_ai.pong_ai #right is chaser ai (before switching)
    
    game_loop(screen, paddles, ball, table_size, clock_rate, turn_wait_rate, score_to_win, 0) #edit display from 0 to 1
    ball = Ball(table_size, ball_size, paddle_bounce, wall_bounce, dust_error, init_speed_mag)
